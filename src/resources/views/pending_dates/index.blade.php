<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('pending_dates.create') }}" class="btn btn-outline-primary px-2 py-1 rounded-md">
                {{ __('Nueva cita') }}
            </a>
        </div>
    </x-slot>
                                    ...
                                    <tr>
                                        ...
                                        <td class="border border-gray-300 px-4 py-2 w-1/6 text-center"
                                            <div class="flex justify-center items-center gap-1">
                                                <a href="{{ route('pending_dates.show', $date) }}" class="btn btn-sm btn-outline-primary" title="{{ __('Ver') }}">
                                                    <i class="bi bi-eye-fill"></i>
                                                </a>

                                                <a href="{{ route('pending_dates.edit', $date) }}" class="btn btn-sm btn-outline-success" title="{{ __('Editar') }}">
                                                    <i class="bi bi-pencil-fill"></i>
                                                </a>

                                                <form action="{{ route('pending_dates.destroy', $date) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
